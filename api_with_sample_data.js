...
fs.readFile('sample_dataset.json', 'utf8', (err, data) => {
  if (err) {
    ...
    ohlcData = JSON.parse(data);
    console.log('Sample dataset loaded successfully.');
  }
});

// Helper Function to Calculate OHLC Average
function calculateOHLC(data) {
  const length = data.length;
  if (length === 0) return null;

  // Calculate the sum of all open, high, low, and close values in the dataset
  const sum = data.reduce((acc, item) => acc + (item.open + item.high + item.low + item.close), 0);
  // Calculate the average by dividing the sum by the total number of values (4 times the number of records)
  const avg = sum / (length * 4);
  return avg;
}
...
app.get('/api/average', (req, res) => {
  ...
  if (window === 'last_5_items') {
    ...
    const average = calculateOHLC(lastFiveData);
    res.json({ movingAverage: average });
    ...
    const average = calculateOHLC(filteredData);
    res.json({ movingAverage: average });
  } else {
